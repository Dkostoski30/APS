...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> najdobra = firstSorcererCards.getFirst();
        for (SLLNode<Card> current = firstSorcererCards.getFirst().succ; current != null; current = current.succ){
            if(najdobra.element.significance()<current.element.significance()){
                najdobra = current;
            }
        }
        ...
        for (SLLNode<Card> current = secondSorcererCards.getFirst(); current != null; numOfNodes++, current=current.succ) {
            if(numOfNodes == secondSorcererCards.size()/2){
                secondSorcererCards.insertBefore(najdobra.element, current);
            }
        }
    }
