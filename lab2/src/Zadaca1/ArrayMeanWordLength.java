...
public class ArrayMeanWordLength {
    ...
    public static double avgLength(Array<String> arr){
        double sum = 0;
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        String rez = arr.get(0);
        double avgLength = avgLength(arr);
        for (int i = 0; i < arr.getSize(); i++) {
            if(Math.abs(arr.get(i).length()-avgLength)<Math.abs(rez.length()-avgLength)){
                rez = arr.get(i);
            }else if(Math.abs(arr.get(i).length()-avgLength)==Math.abs(rez.length()-avgLength)){
                if(arr.get(i).length()>rez.length()){
                    rez = arr.get(i);
                }
            }
        }
